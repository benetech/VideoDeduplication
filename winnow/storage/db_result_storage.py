...
class DBResultStorage:
    ...
    def __init__(self, database):
        ...
        Args:
            database {db.Database}: A database instance to store and fetch results.
        """
    ...
    def add_matches(self, entries):
        ...
        Args:
            entries: Iterable of (path_1,sha256_1,path_2,sha256_2,distance) tuples.
        """
        # Split the work into chunks
        for chunk in chunks(entries, size=1000):
            with self.database.session_scope() as session:
    ...
    def _update_metadata(metadata_entity, metadata):
        ...
        metadata_entity.video_length = metadata.get('video_length', metadata_entity.video_length)
        metadata_entity.avg_act = metadata.get('avg_act', metadata_entity.avg_act)
        metadata_entity.video_avg_std = metadata.get('video_avg_std', metadata_entity.video_avg_std)
        metadata_entity.video_max_dif = metadata.get('video_max_dif', metadata_entity.video_max_dif)
        metadata_entity.gray_avg = metadata.get('gray_avg', metadata_entity.gray_avg)
        metadata_entity.gray_std = metadata.get('gray_std', metadata_entity.gray_std)
        ...
        metadata_entity.video_duration_flag = metadata.get('video_duration_flag', metadata_entity.video_duration_flag)
    ...
    def _matches_file_identifiers(connections):
        """Get file identifiers from connections of the form (path_1,sha256_1,path_2,sha256_2)."""
        result = []
    ...
    def _files_for_matches(session, connections):
        """Get or create files for connections of the form (path_1,sha256_1,path_2,sha256_2)."""
