...
import pandas as pd
from .scene_summary import get_video_summary_list
...
class DuplicateDetector:
    ...
    def find(self, video_signatures):
        ...
        print('Number of Files:{} - Number of Clusters:{}'.format(
                                                video_signatures.shape[0],
                                                len(np.unique(self.clusters))))
        self.df = pd.DataFrame({
                    'signature_fp': self.file_index,
                    'cluster': self.clusters})

        self.df['file_name'] = (self.df['signature_fp']
                                .apply(
                                    lambda x: x.split('/')[-1]
                                    .split('.')[0]
                                    .split('_vgg')[0]))
    ...
    def show_distance_matrix(self, comparison=False):
        ...
        print('White Square should show concentrations of duplicated docs')
    ...
    def attach_source_videos(self, fp, column='fp'):
        ...
        video_list['file_name'] = (video_list['fp'].apply(pars))
