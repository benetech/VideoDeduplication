...
class BaseProgressMonitor(abc.ABC):
    @abc.abstractmethod
    def scale(self, total_work, unit: str = ""):
    ...
    @abc.abstractmethod
    def subtask(self, work_amount) -> "BaseProgressMonitor":
...
class ProgressMonitor(BaseProgressMonitor):
    ...
    def __init__(self, observer=(), scale=1.0):
        self._observers = self._init_observers(observer)
        self._total_work = float(scale)
        self._work_done = 0.0

    def _init_observers(self, observer):
...
class ProgressBar(BaseProgressMonitor):
    ...
    def scale(self, total_work, unit: str = "it"):
...
class LazyProgress(BaseProgressMonitor):
    ...
    @staticmethod
    def _suggest_portion(total_work: Number) -> Number:
    ...
    def __init__(self, delegate: ProgressMonitor, portion: Number = None):
        self._accumulator = 0
