...
use_store = pytest.mark.parametrize("store", [legacy.LMDBReprStorage, legacy.SQLiteReprStorage], indirect=True)


def make_key():
    """Make some repr storage key."""
    unique = uuid()
    return legacy.ReprKey(path=f"some/path-{unique}", hash=f"some-hash-{unique}", tag=f"some-tag-{unique}")
