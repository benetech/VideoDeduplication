...
def add_files(session, file_paths):
    ...
    session.add_all([Files(sha256=x[0], file_path=x[1]) for x in zip(list(hashes), list(file_paths))])
...
def add_signatures(session, signatures, file_ids):
    ...
    """
    session.add_all([Signature(file_id=x[0], signature=x[1]) for x in zip(list(file_ids), list(signatures))])
