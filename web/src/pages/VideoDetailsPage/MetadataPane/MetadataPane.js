...
function useMessages() {
  const intl = useIntl();
  return {
    expand: intl.formatMessage({ id: "actions.expandAll" }),
    collapse: intl.formatMessage({ id: "actions.collapseAll" }),
...
function MetadataPane(props) {
  ...
  const [sectionState, setSectionState] = useState({ open: false });
  const {
    value: search,
  ...
  const expandAll = useCallback(() => setSectionState({ open: true }));
  const collapseAll = useCallback(() => setSectionState({ open: false }));
